...
calculate_metrics <- function(pred_df) {
  ...
  pred_df %>%
    ...
    summarise(
      ...
      biais_rel = mean(abs(bias_rel)),
      relative_rmse = sqrt(mean(bias_rel_square)),
      rmse = sqrt(mean(biais_brut_sqr)),
      biais_out_20percent = mean(abs(bias_rel) > 0.2),
      nb_out_20percent = sum(abs(bias_rel) > 0.2),
      n = n()
    ) %>%
    mutate(
...
test_combinations <- function(data, time_predictors, fixed_predictors = NULL, response,
  ...
  for (n in combo_sizes) {
    ...
    for (combo in time_combos) {
      ...
      # Recipe
      rec <- recipe(as.formula(paste(response, "~ .")), data = train) %>% #####################train#####################
...
results <- test_combinations(
  data = data,
  time_predictors = c("C0","C05","C1", "C2","C3", "C4"),
  fixed_predictors = c(),
  response = "AUCt",
  external_dfs = list(
    "Fernanda" = df2,
